...
public class separateListWorkerRunnable implements Runnable{
	 ...
	 public static volatile List<Triple> sharedList = new ArrayList<Triple>();//to hold client IPs and requested files
	 ...
	 List<Triple> myList = ThreadLocalList.get();
		...
		public void run() {
	        try {
	        	...
	        	if(read==true && !hasUpdated.get(me)){
	        		lock.lock();
	        		try{
	        			sharedList.addAll((Collection<? extends Triple>) myList);//append local list to shared IP list
	        			myList.clear();//clear my list within myList
		...
		private void requestParse(BufferedReader request, DataOutputStream response, String ip) throws IOException{
			...
			Triple element = new Triple();//to store IP and file request
			...
			//get requested resource
			url = reqArr[1];
			//add IP as first element of triple:
			element.str1 = ip;
			//add url as second element of triple:
			element.str2 = url;
			//add timestamp as third element of triple:
			element.ts = new java.sql.Timestamp(Calendar.getInstance().getTime().getTime());
			//add triple to local list:
			myList.add(element);
